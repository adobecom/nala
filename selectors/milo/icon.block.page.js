...
export class Icon {
  constructor(page) {
    ...
    this.cssProperties = {
      ...
      '.con-block.xl-spacing-static-top': {
        'padding-top': '104px',
      },
    };
  ...
  async verifyIcon(iconType, data) {
    ...
    switch (iconType) {
      case 'icon block (fullwidth, medium)':
        ...
        expect(await WebUtil.verifyAttributes(await this.icon,this.attProperties['icon-fullwidth-medium'])).toBeTruthy();

        expect(await WebUtil.verifyCSS(await this.icon,this.cssProperties['.icon-block'])).toBeTruthy();
      ...
      case 'icon block (fullwidth, medium, intro)':
        ...
        // verify icon block attributes and css
        expect(await WebUtil.verifyAttributes(await this.icon,this.attProperties['icon-fullwidth-medium-intro'])).toBeTruthy();

        expect(await WebUtil.verifyCSS(await this.icon,this.cssProperties['.icon-block'])).toBeTruthy();
        expect(await WebUtil.verifyCSS(await this.icon,this.cssProperties['.con-block.xl-spacing-static-bottom'])).toBeTruthy();
        expect(await WebUtil.verifyCSS(await this.icon,this.cssProperties['.con-block.xl-spacing-static-top'])).toBeTruthy();
      ...
      case 'icon block (fullwidth, large)':
        ...
        // verify icon block attributes and css
        expect(await WebUtil.verifyAttributes(await this.icon,this.attProperties['icon-fullwidth-large'])).toBeTruthy();
