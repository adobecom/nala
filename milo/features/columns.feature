Feature: Validate Columns blocks on library document page
  ...
  Scenario Outline: Validating the Columns first, last record text and columns, rows amounts when a style is set on the block
    Given I go to "<url>" columns block page
  ...
  Scenario Outline: Validating the Columns first, last record text and columns, rows amounts when a style is set on the block for bacom
    Given I go to "<url>" columns block page
    When I select the block "<name>"
    Then I should see "<numberColums>" columns
    And I should see "<numberRows>" rows
  ...
  Scenario Outline: Validating the Columns first, last record text and columns, rows amounts when a style is set on the block for blog_acom
    Given I go to "<url>" columns block page
    When I select the block "<name>"
    Then I should see "<numberColums>" columns
