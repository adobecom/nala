      ...
      async ({ page }, testInfo) => {
        ...
        for (const key of Object.keys(testdata)) {
          ...
          await test.step(`Stable: Select each answer on test page according to ${key}`, async () => {
            await stablePage.clickEachAnswer(stableURL, key, keyNumber, 'stable', project, folderPath, true);
          });

          await test.step('Stable: Check results on test page', async () => {
            await stablePage.checkResultPage(testdata[key], key, keyNumber, 'stable', project, folderPath, true);
          });
          ...
          await test.step(`Beta: Select each answer on test page according to ${key}`, async () => {
            await betaPage.clickEachAnswer(betaURL, key, keyNumber, 'beta', project, folderPath, true);
          });

          await test.step('Beta: Check results on test page', async () => {
            await betaPage.checkResultPage(testdata[key], key, keyNumber, 'beta', project, folderPath, true);
          });
