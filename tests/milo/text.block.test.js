...
test.describe('Milo Text Block test suite', () => {
  ...
  test(`${features[0].name},${features[0].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-2: Verify Text specs', async () => {
      ...
      expect(await webUtil.verifyCSS_(text.headline, text.cssProperties['heading-l'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.bodyM, text.cssProperties['body-m'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.actionAreaLink, text.cssProperties['body-m'])).toBeTruthy();
    });
  ...
  test(`${features[2].name},${features[2].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-2: Verify Text (full width) specs', async () => {
      ...
      expect(await webUtil.verifyAttributes_(await text.textFullWidth, text.attProperties['text-full-width'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(await text.fullWidthHeadline, text.cssProperties['heading-l'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(await text.bodyM, text.cssProperties['body-m'])).toBeTruthy();
    });
  ...
  test(`${features[3].name},${features[3].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-2: Verify Text (full-width, large) specs', async () => {
      ...
      expect(await webUtil.verifyAttributes_(text.textFullWidthLarge, text.attProperties['text-full-width-large'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.fullWidthLargeHeadline, text.cssProperties['heading-xl'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.bodyM, text.cssProperties['body-m'])).toBeTruthy();
    });
  ...
  test(`${features[4].name},${features[4].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-2: Verify Text (long form, large) specs', async () => {
      ...
      expect(await webUtil.verifyAttributes_(text.textLongFormLarge, text.attProperties['text-long-form-large'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.longFormDetailL, text.cssProperties['detail-l'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.longFormLargeHeadline, text.cssProperties['heading-l'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.bodyL, text.cssProperties['body-l'])).toBeTruthy();
    });
  ...
  test(`${features[5].name},${features[5].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-2: Verify Text (inset, large, m spacing) specs', async () => {
      ...
      expect(await webUtil.verifyAttributes_(text.textInsetLargeMSpacing, text.attProperties['text-inset-medium-m-spacing'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.insetLargeMSpacingHeadline, text.cssProperties['heading-m'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.bodyL, text.cssProperties['body-l'])).toBeTruthy();
      expect(await webUtil.verifyCSS_(text.propertiesHeadingM, text.cssProperties['heading-m'])).toBeTruthy();
    });
