...
import { Draft } from '../../selectors/helpx/draft.selectors.js';
...
test.describe('Draft sanity test suite', () => {
  ...
  test(`${features[0].name}, ${features[0].tags}`, async ({ page, baseURL }) => { 
    console.info(`[Draft] Checking page: ${baseURL}${features[0].path}`);
  ...
  expect(await WebUtil.verifyCSS(await this.procedure, this.cssProperties['procedure'])).toBeTruthy();
  expect(await WebUtil.verifyCSS(await this.beforeAftr, this.cssProperties['beforeAftr'])).toBeTruthy();
  expect(await WebUtil.verifyCSS(await this.codeBlock, this.cssProperties['codeBlock'])).toBeTruthy();
  expect(await WebUtil.verifyCSS(await this.generic, this.cssProperties['generic'])).toBeTruthy();

  expect(await WebUtil.verifyAttributes(await this.procedure, this.attProperties['procedure'])).toBeTruthy();
  expect(await WebUtil.verifyAttributes(await this.beforeAftr, this.attProperties['beforeAftr'])).toBeTruthy();
  expect(await WebUtil.verifyAttributes(await this.codeBlock, this.attProperties['codeBlock'])).toBeTruthy();
  expect(await WebUtil.verifyAttributes(await this.generic, this.attProperties['generic'])).toBeTruthy();
