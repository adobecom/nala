...
Given(/^I go to "([^"]*)" bypassing the cache$/, iGoToPathBypassCache);

Then(
  /^I should see the same as the cached content$/,
  iShouldSeeTheSameAsCachedContent
);
...
Then(
  ...
  iShouldSeeMyPersonalProfileInGlobalHeader
);

Then(
  /^I should see my personal profile in (DocumentCloud|CreativeCloud) header$/,
  iShouldSeeMyPersonalProfileInCloudHeader
);
...
Then(
  ...
  iTakeAScreenshotAndSaveAsFilename
);

Then(
  /^I take a full screenshot and save as "([^"]*)"$/,
  iTakeAFullScreenshotAndSaveAsFilename
);

Then(
  /^I take a full screenshot with width (\d+)$/,
  iTakeAFullScreenshotWithWidthWidth
);
...
Then(
  ...
  iUpdateScreenshotsResultsIntoRecordingFileFilename
);
...
Then(/^I should see "([^\"]*)" in current url$/, iShouldSeeUrlInCurrentUrl);

Then(
  /^I should see query parameter "([^\"]*)" with value "([^\"]*)" in current url$/,
  iShouldSeeQueryParamWithValueInCurrentUrl
);

Then(
  /^I should not see "([^\"]*)" in current url$/,
  iShouldNotSeeUrlInCurrentUrl
);
...
Then(
  /^I should see the canonical link "([^"]*)" on "([^"]*)"$/,
  iShouldSeeTheCanonicalLink
);
 ...
 * Step Definition:
 * ```
 * /^I go to "([^"]*)" bypassing the cache$/
 * ```
 * @param {string} path Path to go to
 */
function iGoToPathBypassCache(path) {
  this.page = new GnavPage();
  let url = new URL(path, 'https://www.adobe.com');
  url.searchParams.set('disableHTMLComp', Math.floor(Math.random()*1000000));
  this.page.open(url.pathname + url.search);
  console.log(browser.getUrl());
}
 ...
 * ```
 */
function iShouldSeeTheSameAsCachedContent() {
  let pageUrl = browser.getUrl();

  let url = new URL(pageUrl);
  url.searchParams.delete('disableHTMLComp');

  // The test could be run on BrowserStack in different region
  // Don't use Axios requests
  browser.url('view-source:' + pageUrl);
  browser.pause(2000);
  let elem = browser.$("body table");
  let freshSource = elem.getText();

  browser.url('view-source:' + url.href);
  browser.pause(2000);
  elem = browser.$("body table");
  let cachedSource = elem.getText();

  // let userAgent = browser.execute(() => window.navigator.userAgent);
  // userAgent = 'curl/7.79.1';

  // let resFressh = requestGet(pageUrl, { 
  //     headers: {
  ...
  //   });
  // let freshSource = resFressh.data;

  // let resCached = requestGet(url.href, { 
  //   headers: {
  //     'user-agent': userAgent,
  //     'accept': '*/*'
  //   },
  //   transformResponse: (r) => r 
  // });
  // let cachedSource = resCached.data;

  //fs.writeFileSync('cachedSource.html', cachedSource);
  //fs.writeFileSync('freshSource.html', freshSource);

  // exclude Akamai boomerange script, which contains unique ids.
  cachedSource = cachedSource.split('\n').filter(x => !x.includes('https://s.go-mpulse.net/boomerang/') && !x.includes('<script>(window.BOOMR_mq='));
  freshSource = freshSource.split('\n').filter(x => !x.includes('https://s.go-mpulse.net/boomerang/') && !x.includes('<script>(window.BOOMR_mq='));

  if (cachedSource.length === freshSource.length) {
    let cachedSourceDiff = [];
    let freshSourceDiff = [];   
    for (let i=0; i<cachedSource.length; i++) {
      if (cachedSource[i] !== freshSource[i]) {
        let cachedGeoMap = cachedSource[i].match(/.+data-geomap="([^"]+)"/);
        let freshGeoMap = freshSource[i].match(/.+data-geomap="([^"]+)"/);
        if (cachedGeoMap && freshGeoMap) {
          cachedGeoMap = JSON.parse(he.decode(cachedGeoMap[1]));
          freshGeoMap = JSON.parse(he.decode(freshGeoMap[1]));
          try {
            expect(cachedGeoMap).toEqual(freshGeoMap);
            continue;
          } catch (e) {
            console.log("georouting head is different.");
            console.log(e.message);
          }
        }
        cachedSourceDiff.push(cachedSource[i]);
        freshSourceDiff.push(freshSource[i]);
      }
    }
    expect(cachedSourceDiff).toEqual(freshSourceDiff);
  } else {
    expect(cachedSource).toEqual(freshSource); 
  }
}
...
function clickOnCta(label) {
  let elem = browser.$(
    `//*[contains(@class,"cta")]//*[@class="spectrum-Button-label" and text()="${label}"] | ` +
      `//*[contains(@class,"dexter-Cta")]//*[text()[contains(.,"${label}")]] | ` +
      `//*[contains(@class,"dexter-Cta")]//*[text()[contains(.,"${label}")]]/ancestor::a`
  );
  elem.click();
}
