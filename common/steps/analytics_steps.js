...
import getWikiAnalyticsData from '../support/functions/getWikiAnalyticsData';
...
Then(
  /^I load expected analytics data from wiki page "([^\"]*)"(| with replacements "([^\"]*)")$/,
  iLoadExpectedAnalyticsDataFromWikiPage
);
 ...
 */
function iLoadExpectedAnalyticsDataFromWikiPage(wikiID, replacements) {
  if (wikiID !== undefined && wikiID !== '') {
    if (this.page.wikiAnalyticsData === undefined) {
      if (!replacements) {
      ...
      this.page.wikiAnalyticsData = getWikiAnalyticsData(
        wikiID,
        replacementsStr2Obj(replacements)
      );
    }
  }
}
...
/**
 * Converts wiki article replacements from string to objects
 * @param {string} wikiID Wiki article ID
 * @param {string} replacements optional
 */
function replacementsStr2Obj(replacements) {
  let result = {};
  if (!replacements) return result;
  const arr = replacements.split(',');
  for (let replacement of arr) {
    const arr2 = replacement.split('=');
    if (arr2[0] == 'domainName' || arr2[1] == '<baseUrl>')
      arr2[1] = browser.config.profile.baseUrl
        .replace('https://', '')
        .replace('www.', '');
